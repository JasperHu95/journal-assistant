...
<div class="p-8">
  <h2 class="font-serif text-2xl text-[#2C2416] mb-6 tracking-wide">{t("tags.title")}</h2>

  <div class="bg-white border border-[#D4C8B0] p-5 mb-6">
    <div class="flex gap-3 items-end">
      <div class="flex-1">
        <label for="tag-name" class="text-xs text-[#6B5E4A] block mb-1.5">{t("tags.name")}</label>
        <input
          id="tag-name"
          bind:value={newName}
      ...
      <div>
        <label for="tag-color" class="text-xs text-[#6B5E4A] block mb-1.5">{t("tags.color")}</label>
  ...
  <div class="space-y-0">
    ...
    {:else}
      <p class="text-sm text-[#6B5E4A] py-12 text-center italic">{t("tags.empty")}</p>
    {/each}
  </div>
</div>
